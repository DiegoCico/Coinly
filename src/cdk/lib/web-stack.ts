...
export interface WebStackProps extends cdk.StackProps {
  stage: { name: string };           // prod | beta | dev
  serviceName?: string;
...
export class WebStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: WebStackProps) {
    ...
    const serviceName = props.serviceName ?? "coinly-web";
    ...
    this.distribution = new cloudfront.Distribution(this, "Distribution", {
      ...
      // ===== CloudWatch Log Retention (3 weeks) =====
      logFilePrefix: `${serviceName}-logs`,
      enableLogging: true,
      ...
      logRetention: cdk.aws_logs.RetentionDays.THREE_WEEKS,
    });
    ...
    if (props.frontendBuildPath) {
      ...
      if (fs.existsSync(resolved)) {
        ...
        console.warn(`[WebStack] Skipping — directory not found: ${resolved}`);
      }
    }
